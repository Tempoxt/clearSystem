<template>
  <el-row class="h-full public-table-container">
    <el-col :span="4" class="h-full">
      <div class="h-full">
        <org ref="tree" v-if="treeShow" :option="treeOption" :operate="true" @handlerCb="handlerCb">
    ...
    <el-col :span="20" style="border-left:1px solid rgb(232, 232, 232); height: 100%">
      ...
      <add-doc
        v-if="addDocShow"
        :show="addDocShow"
        :treeOption="addDocTreeData"
        :selectFile="selectFile"
        @closeAddDoc="closeAddDoc"
        @req="getTable"
      ></add-doc>
      ...
      <edit-doc
        v-if="addDocTreeData.length > 0 && editShow"
        :show="editShow"
        :treeOption="addDocTreeData"
        :docData="table.selectArr"
        :selectFile="selectFile"
        @close="closeEditDoc"
        @req="req"
      ></edit-doc>
      ...
      <right
        v-if="addRightTreeData.length && rightShow"
        :show="rightShow"
        :treeData="addRightTreeData"
        :tagData="table.selectArr"
      ...
      <version
        v-if="versionShow"
        :show="versionShow"
        :selectArr="table.selectArr"
        :belongFile="belongFile"
        @close="closeVersion"
      ></version>
      ...
      <invalid
        v-if="cancelShow"
        :show="cancelShow"
        :data="table.selectArr[0]"
        @req="cancelReq"
        @close="closeCancel"
      ></invalid>
      <share
        v-if="shareShow && addDocTreeData.length"
        :show="shareShow"
        :treeData="addRightTreeData"
        :doc="table.selectArr[0]"
        @close="closeShare"
      />

      <file-change
        v-show="addDocTreeData.length && fileShow"
        :selectFile="selectFile"
        :data="addDocTreeData"
        :show="fileShow"
        @close="closeFileChange"
        @init="getTree"
      />

      <edit-file
        v-if="addDocTreeData.length && editFileShow"
        :show="editFileShow"
        :data="addDocTreeData"
        :target="target"
        :belong="belongFile"
...
import mergeImg from "@/components/public/mergeImg";
...
export default {
  ...
  computed: {
    table() {
      // 判断是否问系统文件
      if (
        "type" in this.selectFile === false ||
        this.selectFile.type === 1 ||
    ...
    publishArr() {
      let arr = JSON.parse(JSON.stringify(this.table.selectArr));
      ...
      table1: {
        ...
        header: {
          ...
          left: [
            ...
            {
              type: "selectWith", // 级联选项框 目前该处为时间选择器业务组件 后期优化抽离
              tag: "create_time_end", // 接口绑定字段
              select: {
                value: 0, // 时间类型默认值
                holder: "",
                options: [
  ...
  mounted() {
    // this.$request.get('/doc/mydoc').then(data => {
    ...
    // })
    this.table1Copy = this.table1;
    this.table2Copy = this.table2;
  },
  // inject: ['el'],
  methods: {
    test(data) {
    ...
    async req(query) {
      ...
      let data = await this.$request.get("/doc/mydoc/", {
        params: obj,
      });
    ...
    async getTree() {
      ...
      this.treeOption.treeData.data = tData.data;
      const data = await this.$request.get("/doc/docufolder/", {
        params: {
          nosys: 0,
        },
      });
      this.addDocTreeData = data.data;
    ...
    async getTable({ page = 1 } = {}) {
      ...
      console.log(this.table.currentPage, this.table2.currentPage);
      // this.table1.currentPage = page
      // this.table2.currentPage = page
      obj.page = page;
      this.table.loading = true;
      try {
        const data = await this.$request.get("/doc/mydoc", {
          params: obj,
        });
        this.table.loading = false;
        this.table.main.body = data.data.data_list;
    ...
    searchMerge() {
      let obj = {};
      this.table.header.left[1].value !== "" &&
      this.table.header.left[1].value !== null
        ? (obj.create_time_from = this.table.header.left[1].value[0]) &&
          (obj.create_time_end = this.table.header.left[1].value[1])
        : (obj.create_time_from = "") && (obj.create_time_end = "");
