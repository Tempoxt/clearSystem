<template>
  <el-row class="h-full public-table-container">
    <el-tabs v-model="activeName" @tab-click="clearMerge">
      <el-tab-pane label="我的共享" name="1">
      </el-tab-pane>
      <el-tab-pane label="共享给我的" name="2">
      </el-tab-pane>
    </el-tabs>
    <el-col class="mainwrap">
      <employeeTable v-if="tableShow" :table="table" @handlerCb="handlerCb" @selectChange="selectChange"
        ...
        <template #is_share="{scope}">
          <span v-if="scope.row.is_share === 1" style="color: #A3AFB7">取消</span>
    ...
    <share v-if="shareShow" :show="shareShow" :treeData="addRightTreeData" :doc="table.selectArr[0]" @close="closeShare"
      @req="getTable({page: table.currentPage})" />
  ...
  export default {
        ...
        table1: {
            ...
            left: [
              {
                ...
                data: [
                  ...
                  { text: '取消分享', icon: 'icon-fenxiang', func: 'cancelShows', isSelect: true }
                ]
              },
              {
                type: 'selectWith',
          ...
          main: {
            head: [
              ...
              { type: 'link', keys: 'doc_nam', title: '文档名称', func: 'seek', width: 220, fixed: true },
              ...
              { type: 'slot', keys: 'is_share', title: '分享状态', slotName: 'is_share' },
              {
        ...
        table2: {
            ...
            left: [
              {
                ...
                data: [
                  { text: '查询', icon: 'icon-chaxun', func: 'searchShows' },
                  { text: '下载', icon: 'icon-wenjian', func: 'download', isSelect: true },
                  { text: '分享', icon: 'icon-fenxiang', func: 'shareShows', isSelect: true }
                ]
              },
              {
                type: 'selectWith',
          ...
          main: {
            head: [
              { type: 'index', title: '#', fixed: true },
              { type: 'link', keys: 'doc_nam', title: '文档名称', func: 'seek', width: 220, fixed: true },
              { type: 'text', keys: 'acc_name', title: '分享人' },
              { type: 'text', keys: 'version', title: '版本号' },
              {
                type: 'textRules', keys: 'lev', title: '文件级别', rules: {
                  [1]: '一级文件',
              ...
              { type: 'text', keys: 'cr_time', title: '创建时间', sort: true, width: 160 },
              { type: 'text', keys: 'push_time', title: '发布时间', sort: true, width: 160 },
              { type: 'text', keys: 'share_exipr', title: '分享到期日', sort: true, width: 160 },
              { type: 'slot', keys: 'is_share', title: '分享状态', slotName: 'is_share' },
              { type: 'slot', keys: 'read_status', title: '阅读状态', slotName: 'read_status' },
              {
                type: 'tags', keys: 'status', title: '文档状态', options: {
                  1: { color: 'warning', tip: '未生效' },
                  2: { color: 'success', tip: '生效中' },
    ...
    methods: {
      ...
      async req(query) {
        ...
        let url = '/doc/myshare/'
        if (this.activeName === '1') url = '/doc/myshare/'
        else if (this.activeName === '2') url = '/doc/to/myshare/'
